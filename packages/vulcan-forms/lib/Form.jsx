...
class Form extends Component {
	...
	constructor(props) {
		...
		this.newMutationSuccessCallback = this.newMutationSuccessCallback.bind(
			this,
		);
		this.editMutationSuccessCallback = this.editMutationSuccessCallback.bind(
			this,
		);
	...
	getFieldGroups() {
		...
		let fields = this.getFieldNames().map(fieldName => {
			...
			field.hidden = typeof fieldSchema.hidden === "function"
				? !!fieldSchema.hidden.call(fieldSchema, this.props)
				: fieldSchema.hidden;

			// add label or internationalized field name if necessary (field not hidden)
			if (!field.hidden) {
				// debugger;
				field.label = this.context.intl.formatMessage({
					id: this.props.collection._name + "." + fieldName,
					defaultMessage: fieldSchema.label,
				});
			}

			// add value
			field.value = this.getDocument() &&
				deepValue(this.getDocument(), fieldName)
				? deepValue(this.getDocument(), fieldName)
				: "";

			// convert value type if needed
			if (fieldSchema.type.definitions[0].type === Number)
			...
			if (fieldSchema.autoform) {
				...
				console.warn(
					`Vulcan Warning: The 'autoform' field is deprecated. You should rename it to 'form' instead. It was defined on your '${fieldName}' field  on the '${this
						.props.collection._name}' collection`,
				); // eslint-disable-line
			}

			// replace value by prefilled value if value is empty
			if (fieldSchema.form && fieldSchema.form.prefill) {
				const prefilledValue = typeof fieldSchema.form.prefill === "function"
					? fieldSchema.form.prefill.call(fieldSchema)
					: fieldSchema.form.prefill;
			...
			if (fieldSchema.form && fieldSchema.form.options) {
				field.options = typeof fieldSchema.form.options === "function"
					? fieldSchema.form.options.call(fieldSchema, this.props)
					: fieldSchema.form.options;
			}

			if (fieldSchema.form && fieldSchema.form.disabled) {
				field.disabled = typeof fieldSchema.form.disabled === "function"
					? fieldSchema.form.disabled.call(fieldSchema)
					: fieldSchema.form.disabled;
			}

			if (fieldSchema.form && fieldSchema.form.help) {
				field.help = typeof fieldSchema.form.help === "function"
					? fieldSchema.form.help.call(fieldSchema)
					: fieldSchema.form.help;
			}
			...
			if (fieldSchema.beforeComponent)
				field.beforeComponent = fieldSchema.beforeComponent;
			if (fieldSchema.afterComponent)
				field.afterComponent = fieldSchema.afterComponent;
		...
		let groups = _.compact(
			_.unique(_.pluck(fields, "group"), false, g => g && g.name),
		);
	...
	getFieldNames() {
		...
		let relevantFields = this.getFormType() === "edit"
			? getEditableFields(
					this.getSchema(),
					this.props.currentUser,
					this.getDocument(),
				)
			: getInsertableFields(this.getSchema(), this.props.currentUser);
	...
	getDocument() {
		...
		const document = Object.assign(
			currentDocument,
			_.clone(this.state.autofilledValues),
		...
		return (
			<div className="form-errors">
				{this.state.errors.map((message, index) =>
					<Flash key={index} message={message} />,
				)}
			</div>
		);
	}
	...
	submitForm(data) {
		...
		if (this.props.formType === "new") {
			...
			unsetKeys = _.unique(
				unsetKeys.concat(_.difference(this.state.deletedValues, setKeys)),
			);
				...
				>
					...
					<div className="form-submit">
						...
						{this.props.cancelCallback
							? <a className="form-cancel" onClick={this.props.cancelCallback}>
				...
				{this.props.formType === "edit" && this.props.showRemove
					? <div>
							...
							<a
								...
								className={`${collectionName}-delete-link`}
							>
